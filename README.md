...
You'll need OpenJDK 1.8, the Android SDK, the NDK (r25c), and Android-specific CMake. The native code fetches and links against mbedTLS for secure WebSocket connections, so no extra TLS packages are required. The Android manifest already requests Internet access, blocks cleartext traffic, and uses a network security config to trust system certificates, allowing `wss://` endpoints to work out of the box.
